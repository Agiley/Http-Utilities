class Proxy < ActiveRecord::Base
  scope :should_be_checked, lambda { |public_proxies, date, limit| 
  ...
  def self.get_random_proxy(protocol = :all, type = :all)
    proxy           =   nil
  ...
  def self.format_proxy_address(proxy_host, proxy_port = 80, include_http = false)
    proxy_address = "#{proxy_host}:#{proxy_port}"
    proxy_address.insert(0, "http://") if (include_http && !proxy_address.start_with?("http://"))
    return proxy_address
  end
  
  def self.format_proxy_credentials(username, password)
